...
// Much of this has been adapted from https://web.archive.org/web/20190201143324/https://hardy.uhasselt.be/Toga/book_format.html

export default class Polyglot {
    private _entries: Map<bigint, entry> | null = null;

	private readonly _filePath = path.join(__dirname, "khepri_polyglot.bin");
	...
	TryLoad() {
		if (fs.existsSync(this._filePath)) {
			...
			while (i < buffer.length) {
				...
				const prettymove = `${Square[((move & 0xfc0) >> 6) ^ 56]}${Square[(move & 0x3f) ^ 56]}`;
				const promotionPiece = move >> 12;
				let promotionCharacter = "";
				...
				const hash = buffer.readBigUInt64BE(i);

				if (this._entries.has(hash)) {
					const value = this._entries.get(hash) as entry;

					if (value.weight < buffer.readUInt16BE(i + 10)) {
						value.move = `${prettymove}${promotionCharacter}`;
						value.weight = buffer.readUInt16BE(i + 10);
						this._entries.set(hash, value);
					}
				}
				else {
					this._entries.set(hash, {
						move: `${prettymove}${promotionCharacter}`,
						weight: buffer.readUInt16BE(i + 10),
					});
				}
    ...
    static PolyglotHash(position: BoardState) {
        ...
        for (let [index, piece] of position.Squares.entries()) {
			...
			let kind_of_piece = piece.Type * 2;
			if (piece.Color === Color.White)
				kind_of_piece += 1;

            hash ^= random64[64 * kind_of_piece + (index ^ 56)];
        }

        if (position.EnPassSq !== Square.no_sq) {
            hash ^= random64[772 + (position.EnPassSq % 8)];
        }
    ...
    TestHashing() {
        const testPositions = [
            { fen: "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", hash: 0x463b96181691fc9cn },
            { fen: "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1", hash: 0x823c9b50fd114196n },
            { fen: "rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 2", hash: 0x0756b94461c50fb0n },
            { fen: "rnbqkbnr/ppp1pppp/8/3pP3/8/8/PPPP1PPP/RNBQKBNR b KQkq - 0 2", hash: 0x662fafb965db29d4n },
            { fen: "rnbqkbnr/ppp1p1pp/8/3pPp2/8/8/PPPP1PPP/RNBQKBNR w KQkq f6 0 3", hash: 0x22a48b5a8e47ff78n },
            { fen: "rnbqkbnr/ppp1p1pp/8/3pPp2/8/8/PPPPKPPP/RNBQ1BNR b kq - 0 3", hash: 0x652a607ca3f242c1n },
            { fen: "rnbq1bnr/ppp1pkpp/8/3pPp2/8/8/PPPPKPPP/RNBQ1BNR w - - 0 4", hash: 0x00fdd303c946bdd9n },
            { fen: "rnbqkbnr/p1pppppp/8/8/PpP4P/8/1P1PPPP1/RNBQKBNR b KQkq c3 0 3", hash: 0x3c8123ea7b067637n },
            { fen: "rnbqkbnr/p1pppppp/8/8/P6P/R1p5/1P1PPPP1/1NBQKBNR b Kkq - 0 4", hash: 0x5c3f9b829b279560n },
        ];
